...
function startLoad(cookie) {
  ...
  if (token) {
    ...
    loadHtml(
      ...
      "",
      "afterbegin",
      "../../../assets/js/admin/dashboard/dashboard.js"
    );
    getAccount();
    listCategory();
  } else {
...
function getAccount() {
  getConnectAPI(
    "GET",
    `https://hanoifoodbank.herokuapp.com/api/v1/hfb/users/${currentUserName}`,
    null,
    function (result) {
...
function addActive(ele) {
  ...
  if (siblings && siblings.length > 0) {
    for (
      let index = 0, len = siblings.length;
...
function listCategory() {
  getConnectAPI(
    "GET",
    "https://hanoifoodbank.herokuapp.com/api/v1/hfb/categories?status=1",
    null,
    function (result) {
...
function goBack(parentFile, file) {
  ...
  loadHtml(
    "../../../inc/layout/admin/content/" + parentFile + "/" + file + ".html",
    ".page-wrapper",
    "div",
    "page-content",
    "",
