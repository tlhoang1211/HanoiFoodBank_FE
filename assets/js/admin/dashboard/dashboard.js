...
function drawChart() {
  ...
  getConnectAPI(
    "POST",
    "https://hanoifoodbank.herokuapp.com/api/v1/hfb/statistics/donation",
  ...
  getConnectAPI(
    "POST",
    "https://hanoifoodbank.herokuapp.com/api/v1/hfb/statistics/food",
    JSON.stringify(dataPost),
    function (result) {
      if (result && result.status == 200) {
        if (result.data && result.data.length > 0) {
  ...
  getConnectAPI(
    "POST",
    "https://hanoifoodbank.herokuapp.com/api/v1/hfb/statistics/request",
    JSON.stringify(dataPost),
    function (result) {
      if (result && result.status == 200) {
        if (result.data) {
...
function initDaterangepicker(alwaysShowCalendars) {
  $("#rs-date")
    ...
    .on("hide.daterangepicker", function (ev, picker) {
      var fromDate_temp = new Date(
        picker.startDate.format("YYYY-MM-DD HH:mm")
      ).setHours(0, 0, 0, 0);
      var toDate_temp =
        new Date(picker.endDate.format("YYYY-MM-DD HH:mm")).setHours(
          0,
          0,
          0,
          0
